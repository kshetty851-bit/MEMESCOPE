...
def walk(start: float, end: float, step_pips: float = 5.0,
         t0: datetime | None = None, minutes: int = 1) -> list[tuple]:
    ...
    n = int(round(abs(end - start) / (step_pips * PIP)))
    out = []
    cur = start
    for i in range(n):
        nxt = round(cur + d, 7)
        out.append((t, round(cur, 7), round(max(cur, nxt), 7),
                    round(min(cur, nxt), 7), nxt))
        cur = nxt
