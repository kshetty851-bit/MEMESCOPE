...
def random_walk(n: int, seed: int, drift: float = 0.0, vol_pips: float = 3.0) -> list[tuple]:
...
def check(e: GridEngine, mid: float) -> None:
    ...
    # total that drifted.
    assert e.used_margin == pytest.approx(sum(p.margin for p in e.positions), abs=1e-9)
...
def test_the_books_balance_over_every_kind_of_path():
    ...
    for seed, drift, vol in (
        ...
        assert e.balance == pytest.approx(e.cfg.start_equity + realised + swap, abs=1e-6), (
...
def test_a_rejected_fill_never_consumes_the_margin_it_was_refused():
    ...
    e = GridEngine(GridConfig(step_pips=15, levels=6, start_equity=300.0), C0, candles[0][0])
    for minute, o, h, lo, c in candles:
        e.step(minute, o, h, lo, c)
        assert e.used_margin == pytest.approx(sum(p.margin for p in e.positions), abs=1e-9)
...
def test_the_neutral_grid_holds_no_short_below_the_centre():
    ...
    e = GridEngine(GridConfig(step_pips=25, levels=4, stop_multiplier=0.0), C0, candles[0][0])
    for minute, o, h, lo, c in candles:
        e.step(minute, o, h, lo, c)
        for p in e.positions:
            if p.origin.level < 0:
...
def test_two_runs_over_an_adversarial_path_are_byte_identical():
    ...
    def run() -> tuple:
        e = GridEngine(
            GridConfig(step_pips=15, levels=6, start_equity=500.0), C0, candles[0][0]
        )
        for minute, o, h, lo, c in candles:
            e.step(minute, o, h, lo, c)
        e.finish(candles[-1][0], candles[-1][4])
