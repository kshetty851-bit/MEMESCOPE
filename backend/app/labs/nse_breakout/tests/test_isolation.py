...
TABLES = ["bt_candles", "bt_index_closes", "bt_ingest_days", "bt_runs", "bt_universe"]
...
#: `bhavcopy.py` is pure parsing over text. If it ever learns a network
#: exists, a rate-limited fetch could start changing what a bar says.
PURE_MODULES = ("bhavcopy.py",)


def migration() -> pathlib.Path:
    """Found by content, never by filename. Shipping a lab from one branch to
    another renumbers its migration, and a hard-coded name would fail for a
    reason that has nothing to do with the lab."""
    hits = [p for p in (BACKEND / "alembic" / "versions").glob("*.py")
            if "bt_universe" in p.read_text()]
...
def test_the_parser_never_learns_a_network_exists() -> None:
    for name in PURE_MODULES:
        leaked = {m for m in imported_modules(ast.parse((PACKAGE / name).read_text()))
                  if m.startswith(("httpx", "app.db", "app.services",
...
def test_the_migration_is_purely_additive() -> None:
    ...
    created = sorted(n.args[0].value for n in calls if n.func.attr == "create_table")
    ...
    assert all(t.startswith(("bt_", "ix_bt_", "uq_bt_")) for t in targets), targets
...
def test_the_migration_matches_the_models() -> None:
    ...
    for model in (models.BtUniverseMember, models.BtCandle, models.BtIndexClose,
                  models.BtIngestDay, models.BtRun):
