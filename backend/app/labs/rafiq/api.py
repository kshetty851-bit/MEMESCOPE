...
class BreakerOut(BaseModel):
    strategy_code: str
    #: True only for D and E. The others are reported for comparison and are
    #: not gated by it.
    gates_entries: bool
    day: str
    day_open_equity: str
...
async def status(session: AsyncSession = Depends(get_db)) -> StatusOut:
    ...
    if not config.enabled():
        return StatusOut(running=False, starting_equity=str(config.STARTING_EQUITY),
                         strategies=[])
    rows, all_positions = await _rows(session)
    ...
    for row in rows:
        ...
        out.append(StrategyOut(
            code=row.code, name=spec.name, lane=row.lane,
            take_profit_mult=str(x.take_profit_mult), stop_mult=str(x.stop_mult),
...
async def positions(session: AsyncSession = Depends(get_db)) -> list[PositionOut]:
    ...
    for p in [x for x in all_positions if x.status == "open"]:
        ...
        out.append(PositionOut(
            ...
            entry_price=str(p.entry_price), quantity=str(p.quantity),
            cost_basis=str(p.cost_basis), stop_price=str(p.stop_price),
            stop_pct=str(p.stop_pct), target_price=str(p.target_price),
