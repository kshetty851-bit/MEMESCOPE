    ...
    async def _screen(
        ...
        *,
        ...
        for rank, row in enumerate(rows, start=1):
            ...
            observations.append(
                eligibility.Observation(
                    ...
                    score=row.current_opportunity_score,
                )
            )
        ...
        return eligibility.screen(
            ...
            # Both off by default, and both `None` when off rather than a
            # permissive value: an inapplicable condition and a condition that
            # happens to admit everything read the same in the refusal counts,
            # and only one of them is true.
            eligible_categories=(
                frozenset(settings.PAPER_ELIGIBLE_CATEGORIES)
                if settings.FEATURE_PAPER_CATEGORY_GATE
                else None
            ),
            score_percentile=(
                settings.PAPER_MIN_SCORE_PERCENTILE
                if settings.FEATURE_PAPER_SCORE_PERCENTILE
                else None
            ),
        )
