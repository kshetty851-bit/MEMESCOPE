...
class Settings(BaseSettings):
    ...
    # --- Entry-selectivity experiments (ENTRY-SELECT-1) -------------------
    #
    # Three independent admission tightenings from the 2026-08-23 profitability
    # research, which found the admitted population 62.9% catastrophic within
    # six hours and no exit rule able to survive it. Each ships **off**: the
    # research replayed them against the 245 audited closes and none of them is
    # enabled on that evidence alone. They are separate flags because they act
    # at different layers and the replay scores them separately — turning on
    # two at once and reading one number would measure nothing.

    #: **Flag 1 — Radar admission liquidity floor.**
    #:
    #: Applied as a hard gate in `radar.detector.qualifies`, not as a risk-score
    #: input. Today liquidity reaches admission only through the risk dimension,
    #: where `<= $1,000` scores 10 and `<= $5,000` scores 40 against a floor of
    #: 30 — so the *effective* floor is about $1,000 and a $1,200 pool passes.
    #:
    #: This gates the **Radar and therefore the Track Record**, not just paper
    #: entry: a token below the floor is never detected, so it never enters the
    #: permanent record. That is a deliberate, and wider, choice than a wallet
    #: filter, and it is why the flag exists rather than the constant moving.
    FEATURE_RADAR_LIQUIDITY_FLOOR: bool = False
    RADAR_MIN_LIQUIDITY_USD: float = Field(default=25_000.0, ge=0.0)

    #: **Flag 2 — category eligibility for paper entry.**
    #:
    #: Restricts new paper entries to the listed detector categories. Radar
    #: detection, the Track Record and every open position are untouched: this
    #: is an entry condition, counted as a refusal like any other, and it can
    #: never stop an exit.
    #:
    #: The default list is the two categories that require independent evidence
    #: to agree — `elite` needs four dimensions above 65, `undervalued` needs
    #: sound structure the market has not moved on. `early_momentum` is the
    #: detector's *fallback* category for anything that qualifies without
    #: matching a specific gate, which is why it is both the largest bucket and
    #: the worst performing one.
    FEATURE_PAPER_CATEGORY_GATE: bool = False
    PAPER_ELIGIBLE_CATEGORIES: tuple[str, ...] = ("elite", "undervalued")

    #: **Flag 3 — score percentile floor for paper entry.**
    #:
    #: Makes entries compete. Today the wallet walks the ranked page and buys
    #: until the cash runs out — 1,955 of 2,295 recorded refusals are
    #: `insufficient_paper_cash`, which means the binding constraint on what the
    #: wallet owns is the order it saw tokens in, not their quality. A
    #: percentile floor is computed across the candidate page at decision time,
    #: so a token qualifies by standing above its peers rather than by arriving
    #: while money was left.
    #:
    #: Percentile of the page's own score distribution, 0-100. `60.0` admits the
    #: top 40%. Computed from the observations that passed every other
    #: condition, so a page of one candidate always clears its own percentile.
    FEATURE_PAPER_SCORE_PERCENTILE: bool = False
    PAPER_MIN_SCORE_PERCENTILE: float = Field(default=60.0, ge=0.0, le=100.0)
