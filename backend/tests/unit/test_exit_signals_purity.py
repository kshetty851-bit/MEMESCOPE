"""Purity constraints for the Exit Watch engine.
...
PACKAGE = Path(__file__).resolve().parents[2] / "app" / "exit_signals"
...
IO_MODULES = {"api.py", "schemas.py"}

#: Modules outside the package the pure engine may reach for.
# Exit Watch reads the Radar's series type and normalisers rather than
#: duplicating them — one definition of an observation window, not two.
ALLOWED_APP_MODULES = {"app.exit_signals", "app.radar"}
