...
export function StockPanel({
  ...
  // This panel renders BELOW the board, and the board is a hundred rows long:
  // without scrolling to it, clicking a stock opens a chart the reader never
  // sees and the click reads as broken. That is how it was reported.
  //
  // **Keyed on the data arriving, not on the symbol changing.** While the
  // fetch is in flight this is a short skeleton; scrolling to it then, and
  // letting the real content replace it a moment later, lands the reader in
  // the middle of a chart with the heading already above them — which is what
  // the first version of this did.
  //
  // **Instant, not smooth.** `behavior: "smooth"` is silently dropped in some
  // browsers and embedded views — measured: identical call, smooth moved the
  // container 0px where auto moved it 4,577 — and a scroll that sometimes does
  // nothing is the bug this exists to fix. A jump of four thousand pixels past
  // a hundred rows reads better instantly anyway, and instant is what a reader
  // who asked for reduced motion wants regardless.
  ...
  // Wrapped rather than a ref on `Panel`: that component does not forward one,
  // and it belongs to the whole repo — this lab edits nothing outside itself.
  return (
        ...
        <PanelHeader className="flex items-center justify-between px-4 pt-3">
          <PanelTitle>
            ...
            {query.data?.stock.name ? (
              <span className="ml-2 font-normal text-ink-3">{query.data.stock.name}</span>
            ) : null}
          </PanelTitle>
          <button
            type="button"
          ...
          <div className="space-y-4 p-4">
              ...
              <div className="space-y-3">
                {query.data.score ? (
                  ...
                  <p className="text-sm text-ink-3">
                    No score: fewer than 250 daily bars, so this stock is in the universe
                ...
                <dl className="grid grid-cols-2 gap-x-4 gap-y-2 border-t border-line pt-3">
                  ...
                  <Field label="20d range" value={plainPct(query.data.levels?.range_pct)} />
                  <Field label="Turnover" value={crore(query.data.stock.turnover_20d)} />
            ...
            <div>
              ...
              {query.data.history.length === 0 ? (
                <p className="text-sm text-ink-3">
                  No setup has opened on this stock yet. Episodes open on the first WATCH or
                  NEAR bar.
                </p>
              ) : (
