...
function buildScene(
  ...
  // grid; it may not lose either of these.
  {
    ...
    items.push({
      ...
      // "vault-door", not "vault": Vault is a zone AND an employee, and his
      // own anchor lands in this same `scene` array, so the bare id collided
      // with him and React silently dropped one of the two on every reconcile.
      //
      // Found independently on two branches, which is how a bug that only
      // warns during an *update* eventually gets caught — a single render and
      // a console read after page load both come back clean with it still live.
