...
export function StrategyCard({ strategy }: { strategy: PaperStrategy }) {
  return (
    <Panel density="compact" className="border-line/70">
      <div className="flex flex-wrap items-baseline justify-between gap-2">
        <div>
          <Label>The rule</Label>
          <p className="mt-1 text-heading font-medium text-ink">
            {strategy.name}{" "}
            <span className="text-xs font-normal text-ink-faint">v{strategy.version}</span>
          </p>
        </div>
        {!strategy.operational && strategy.unavailable_reason ? (
          <span className="rounded-chip border border-line px-2 py-0.5 text-label uppercase tracking-wide text-ink-faint">
            Not running
          </span>
        ) : null}
      </div>
      ...
      {strategy.unavailable_reason ? (
        <p className="mt-3 text-xs text-ink-faint">{strategy.unavailable_reason}</p>
      ) : null}
    </Panel>
  );
}
