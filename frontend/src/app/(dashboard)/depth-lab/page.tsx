        ...
        <div className="mt-2 overflow-x-auto">
          <table className="w-full text-left font-mono text-[11px]">
            <thead className="text-muted">
              <tr>
                <th className="py-1 pr-3 font-normal">floor</th>
                <th className="py-1 pr-3 font-normal">cell</th>
            ...
            <tbody className="text-ink">
              {cells.map((c) => (
                <tr key={c.strategy_id} className="border-t border-line">
                  <td className="py-1 pr-3">{floorLabel(c.floor_usd)}</td>
                  <td className="whitespace-nowrap py-1 pr-3">
                    {c.strategy_id} {c.name}
                  </td>
                  <td className={`py-1 pr-3 text-right ${toneOf(Number(c.equity) - start)}`}>
                    {money(c.equity)}
                  </td>
                  <td className="py-1 pr-3 text-right">{money(c.cash)}</td>
                  <td className="py-1 pr-3 text-right">{c.open_positions}</td>
                  <td className="py-1 pr-3 text-right">{c.cycles_banked}</td>
                  <td className="py-1 pr-3 text-right">{c.cycle_no ?? "—"}</td>
                  <td className="py-1 text-right">{money(c.target_usd)}</td>
                </tr>
              ))}
            </tbody>
          </table>
